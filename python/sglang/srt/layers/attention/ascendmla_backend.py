...
"""
Support attention backend for AscendMLA.

#TODO
Enable speculative sampling in AscendMLA
"""
...
class AscendMLADecodeMetadata:
    ascendmla_metadata: Optional[Tuple[torch.Tensor, torch.Tensor]] = None
    ...
    def __init__(
        ...
        self.ascendmla_metadata = ascendmla_metadata
        self.num_splits = num_splits
        self.block_kv_indices = block_kv_indices


class AscendMLABackend(TorchNativeAttnBackend):
    """Ascendmla attention kernels."""
    ...
    def __init__(
        ...
        self.forward_metadata: Optional[AscendMLADecodeMetadata] = None
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            self.forward_metadata = AscendMLADecodeMetadata(
                mla_metadata,
                num_splits,
                block_kv_indices,
            )
            ...
            # Use AscendMLADecodeMetadata which has the attributes forward_extend expects
            self.forward_metadata = AscendMLADecodeMetadata(
                mla_metadata,
                num_splits,
                block_kv_indices,
    ...
    def forward_decode(
        ...
        if self.data_type == torch.float8_e4m3fn:
            ...
            #     causal=True,
            # )
            o = self._run_ascend_forward_decode(
                reshape_q, k_cache, None, layer, forward_batch, save_kv_cache
            )
            return o.view(-1, layer.tp_q_head_num * layer.v_head_dim)
    ...
    def forward_extend(
        ...
        if (
            ...
            return self._run_ascend_forward_extend(
    ...
    def _run_ascend_forward_extend(self, q, k, v, layer, forward_batch, save_kv_cache):
        """
        q: (b*s, N, q_dim=192)
        k: (b*s, N, k_dim=192)
        v: (b*s, N, v_dim=128)
        ...
        _, _, k_dim = k.size()
        assert q_dim == k_dim
        attn_weights = torch.matmul(q, k.transpose(1, 2)) * layer.scaling  # (bs, n, n)
    ...
    def _run_ascend_forward_decode(self, q, k, v, layer, forward_batch, save_kv_cache):
        o = q.new_zeros((q.shape[0], layer.tp_q_head_num, layer.v_head_dim))
        return o

    def _bmm_5d(self, x, y):
        b, s, n, _, d = x.shape
        x = x.view(b * s, n, d).transpose(0, 1)
        output = torch.matmul(x, y)
        return output.transpose(0, 1).view(b, s, n, -1)


# TODO: multi step kv indices optimization
class AscendMLAMultiStepDraftBackend:
    """
    Wrap multiple ascendmla attention backends as one for multiple consecutive
    ...
    def __init__(
        ...
        if topk > 1:
            raise ValueError(
                f"Currently AscendMLA only supports topk=1 for speculative decoding"
            )
        self.topk = topk
